...
namespace BeliefEngine.HealthKit
{
	...
	public partial class HealthStore : MonoBehaviour {
		...
		*/
		...
		/*! @brief 				Read quantity samples & return a enemyConrollerList of QuantitySamples.
			@details
			@param dataType		The datatype to read.
			@param startDate	The date to start reading samples from.
			@param endDate		The end date to limit samples to.
		...
		/*! @brief 				Read category samples & return a enemyConrollerList of CategorySamples.
			@details
			@param dataType		The datatype to read.
			@param startDate	The date to start reading samples from.
			@param endDate		The end date to limit samples to.
		...
		/*! @brief 				Read correlation samples & return a enemyConrollerList of CorrelationSamples.
			@details
			@param dataType		The datatype to read.
			@param startDate	The date to start reading samples from.
			@param endDate		The end date to limit samples to.
		...
		/*! @brief 				Read workout samples & return a enemyConrollerList of WorkoutSamples.
			@details
		...
		/*! @brief 				Write a workout session using a WorkoutBuilder.
			@details
			@param builder      a WorkoutBuilder object.
			@param handler		Called when the function finishes executing.
		 */
		public void BuildWorkout(WorkoutBuilder builder, WroteSample handler) {
			this.builtWorkoutHandler = handler;
			if (this.IsHealthDataAvailable()) {
				string builderString = (builder != null) ? builder.ToXMLString() : null;
#if UNITY_IOS && !UNITY_EDITOR
				_BuildWorkout(builderString);
#elif UNITY_EDITOR
				Debug.Log(builderString);
#endif
			}
			else {
				Debug.LogError("Error: no health data is available. Are you running on an iOS device that supports HealthKit?");
		...
		/*! @brief 				Stops an Observer Query.
			@details
			@param dataType		The datatype of the query to stop.
		 */
		...
		/*! @brief 				Registers a callback for an observer query.
			@details
		...
		public void ReadStatistics(HKDataType dataType, DateTimeOffset startDate, DateTimeOffset endDate, StatisticsOptions options, ReceivedHealthData<HealthStatistics, Error> handler) {
			this.receivedStatisticsHandlers[dataType] = handler;
			this.ReadHealthStatistics(dataType, startDate, endDate, null, options);
		}
		...
		/*! @brief 				Perform a statistics query.
			...
			@param startDate	The date to start reading samples from.
			@param endDate		The end date to limit samples to.
			@ param predicate   search predicate to filter the samples.
			@param options      the statistics options.
			@param handler		Called when the function finishes executing.
		 */
		public void ReadStatistics(HKDataType dataType, DateTimeOffset startDate, DateTimeOffset endDate, Predicate predicate, StatisticsOptions options, ReceivedHealthData<HealthStatistics, Error> handler) {
			this.receivedStatisticsHandlers[dataType] = handler;
			this.ReadHealthStatistics(dataType, startDate, endDate, predicate, options);
		}
		...
		/*! @brief enable background delivery for a datatype. This isn't very useful, TBH... */
		...
		private WroteSample builtWorkoutHandler;
		...
		private void ReadHealthStatistics(HKDataType dataType, DateTimeOffset startDate, DateTimeOffset endDate, Predicate predicate, StatisticsOptions options) {
			if (this.IsHealthDataAvailable()) {
				string identifier = HealthKitDataTypes.GetIdentifier(dataType);
				string startStamp = DateTimeBridge.DateToString(startDate);
				string endStamp = DateTimeBridge.DateToString(endDate);
				...
				Debug.LogFormat("reading quantity statistics from:\n-{0} ({1})\nto:\n-{2} ({3})", startDate, startStamp, endDate, endStamp);
#if UNITY_IOS && !UNITY_EDITOR
				_ReadStatistics(identifier, startStamp, endStamp, predicateString, optionsString);
#elif UNITY_EDITOR
				this._ReadStatistics_DUMMY(identifier, startStamp, endStamp, predicateString, optionsString);
#endif
			}
			else {
				Debug.LogError("Error: no health data is available. Are you running on an iOS device that supports HealthKit?");
		...
		internal void ParseHealthXML(string xmlString) {
			...
			switch (rootName) {
				...
				case "write":
					...
					return;
				case "builder":
					if (this.builtWorkoutHandler != null) {
						this.builtWorkoutHandler(true, xml.error);
						this.builtWorkoutHandler = null;
					}
					return;
				case "combined":
		...
		[DllImport ("__Internal")]
		private static extern void _BuildWorkout(string builderString);
		...
		[DllImport ("__Internal")]
		private static extern void _ReadStatistics(string identifier, string startDate, string endDate, string predicateString, string options);
