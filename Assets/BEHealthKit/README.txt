...
For sleep data, it's probably worth reading the [HKCategoryValueSleepAnalysis](https://developer.apple.com/library/ios/documentation/HealthKit/Reference/HealthKit_Constants/#//apple_ref/c/tdef/HKCategoryValueSleepAnalysis) documentation to understand what you're looking at. Basically a value of 0 means "in bed", and a 1 means "asleep". Other possible values are Awake (2), Core Sleep (3), Deep Sleep (4), and REM sleep (5). 

These *will* overlap, assuming a good HealthKit citizen is writing the data.
...
Predicate support in BEHealthKit is very limited, and missing many of the native convenience methods, but you are able to create Predicates with a format string and 0, 1, or 2 values. This predicate will limit the search to samples that come from an Apple Watch or iPhone, ignoring any from 3rd-party devices:

	Predicate predicate = new Predicate("%K IN %@", "device.HKDevicePropertyModel", new string[] {"Watch", "iPhone"});

The first argument after the format string *must* be a string. For the second, currently only `string`, `List<string>` and `string[]` are supported.
...
There are a few ways to write workout data. The simpler way is to use HealthStore.WriteWorkoutSample(). This comes in 2 forms:

- WriteWorkoutSample(WorkoutActivityType activityType, DateTimeOffset startDate, DateTimeOffset endDate, WroteSample handler)
- WriteWorkoutSample(WorkoutActivityType activityType, DateTimeOffset startDate, DateTimeOffset endDate, Quantity calories, Quantity distance, WroteSample handler)

If you don't have any calorie or distance information & just want to record a workout, the first will do the job. Otherwise, the second method will allow you to record a workout including either energy burned or distance traveled (or both).

While both of these *will* record a workout to the Health Store (visible in the Health app on the phone), they *won't* contribute to the user's fitness rings (phone or watch). To do that, you have to use the [WorkoutBuilder](https://developer.apple.com/documentation/healthkit/hkworkoutbuilder). Here's some basic sample code for how that works:

		WorkoutConfiguration config = new WorkoutConfiguration(WorkoutActivityType.Walking);
		
		DateTimeOffset end = DateTimeOffset.UtcNow;
		DateTimeOffset start = end.AddMinutes(-15);
		WorkoutBuilder builder = new WorkoutBuilder(config, start, end);
		
		WorkoutSample workout = new WorkoutSample(WorkoutActivityType.Walking, start, end);
		workout.totalDistance = new Quantity(0.5, "mi");
		workout.totalEnergyBurned = new Quantity(75, "Cal");
		builder.AddWorkouts(new List<WorkoutSample> { workout });

		this.healthStore.BuildWorkout(builder, delegate (bool success, Error error) {
			// check success/failure
		});

As you can see, a workout session using the WorkoutBuilder can contain one or more separate WorkoutSamples.

**IMPORTANT**: if you use this method to save workouts, you MUST also request permission for 
 - Active Energy Burned (if you include a value for totalEnergyBurned)

and, if you include totalDistance, one of:
 - Swimming Distance (for Swimming, Water Sports or Water Polo)
 - Downhill Snow Sports Distance (for Skiing, Snowboarding, or Snow Sports)
 - Wheelchair Distance (for Wheelchair Run Pace or Wheelchair Walk Pace)
 - Walking/Running Distance (for anything else??)
