...
- (HKUnit *)defaultUnitForSampleType:(HKSampleType *)sampleType
{
	...
	if ([sampleType isKindOfClass:[HKQuantityType class]]) {
		...
		// Count/Time Types
		// ----------------
		if ([self.countTimeTypes containsObject:quantityType.identifier]) {
...
- (HKUnit *)genericUnitForSampleType:(HKSampleType *)sampleType
{
	...
	if ([sampleType isKindOfClass:[HKQuantityType class]]) {
		...
		// Energy
		if ([quantityType isCompatibleWithUnit:[HKUnit largeCalorieUnit]]) return [HKUnit largeCalorieUnit];
		// Length
