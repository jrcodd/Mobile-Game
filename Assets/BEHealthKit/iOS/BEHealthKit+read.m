...
- (void)readCharacteristic:(HKCharacteristicType *)characteristic resultsHandler:(void (^)(id result, NSError *error))resultsHandler
{
	...
	if ([characteristic.identifier isEqualToString:HKCharacteristicTypeIdentifierBiologicalSex]) {
		...
		result = [self.healthStore dateOfBirthComponentsWithError:&error];
	}
...
void _ReadStatistics(char *identifier, char *startDateString, char *endDateString, char *predicateString, char *optionsString) {
	NSString *identifierString = [NSString stringWithCString:identifier encoding:NSUTF8StringEncoding];
	HKQuantityType *sampleType = [HKSampleType quantityTypeForIdentifier:identifierString];
	if (!sampleType) {
		NSLog(@"Error; unknown quantity-type identifier '%@'", identifierString);
	...
	NSPredicate *spanPredicate = [HKQuery predicateForSamplesWithStartDate:startDate endDate:endDate options:HKQueryOptionStrictStartDate];
	NSPredicate *predicate = nil;
	if (predicateString != NULL) {
		predicate = [NSCompoundPredicate andPredicateWithSubpredicates: @[
			spanPredicate,
			[NSPredicate predicateFromXMLString:[NSString stringWithCString:predicateString encoding:NSUTF8StringEncoding]]
		]];
	} else {
